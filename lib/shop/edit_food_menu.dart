...
class EditFoodMenu extends StatefulWidget {
  ...
  const EditFoodMenu(
      {Key key, this.foodModel, this.userModel, this.groupFoodModel})
      : super(key: key);
  @override
...
class _EditFoodMenuState extends State<EditFoodMenu> {
  ...
  FoodModel foodModel;
  File file;
  String nameFood, price, detail, pathImage, idGrp, idShop, status, nameGroup;
  ...
  void initState() {
    ...
    nameGroup = groupFoodModel.nameGroup;
    groupFoodModel = widget.groupFoodModel;
  ...
  Future<Null> confirmEdit() async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('ທ່ານຕ້ອງການຈະປ່ຽນແປງຂໍ້ມູນສິນຄ້ານີ້ແທ້ບໍ ?'),
        children: <Widget>[
          Row(
